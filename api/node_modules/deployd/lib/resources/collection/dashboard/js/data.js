...
(function() {
  ...
  var resource = Context.resourceId
    , isUser = Context.resourceType === "UserCollection", ExtraAPI='api/';
  ...
  vm.view.scrollToRow = function(row, fn) {
    ...
    setTimeout(function() {
      ...
      if (rowIndex === -1) {
        dpd(ExtraAPI+resource).get('index-of', rowId, function(res) {
          var index = res.index
  ...
  function createRow(data) {
    ...
    rowVm._deleteRow = function() {
      if (rowVm.id()) {
        ...
        dpd(ExtraAPI+resource).del(rowVm.id(), {$skipEvents: true}, function(res, err) {
    ...
    rowVm._saveProp = function(prop, value, options) {
      ...
      if (lastValue !== value) {
        ...
        if (rowVm.id()) {
          ...
          dpd(ExtraAPI+resource).put(rowVm.id(), body, function(res, err) {
            if (err) {
  ...
  function postRow(data, fn) {
    ...
    dpd(ExtraAPI+resource).post(data, function(res, err) {
  ...
  function loadCount(fn) {
    dpd(ExtraAPI+resource).get('count', function(res) { 
      vm.count(res.count);
      if (typeof fn === 'function') {
        fn();
      }
  ...
  function loadPage(page, fn) {
    ...
    dpd(ExtraAPI+resource).get({$skip: begin*PAGE_SIZE, $limit: PAGE_SIZE*3, $skipEvents: true}, function(res, err) {
      if (err) return;
      vm.currentPage(page);
